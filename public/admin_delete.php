...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST["confirm"])) {
        // Delete the scooter
        $deleteQuery = $pdo->prepare("DELETE FROM scooter_brands WHERE id = :id");
        $deleteQuery->bindParam(":id", $id);
        $deleteQuery->execute();

        $_SESSION['message'] = "De scooter is succesvol verwijderd.";
        header("location:admin.php");
        exit;
...
<div class="container mt-5">
    <h1 class="text-danger">Verwijder scooter met ID <?= htmlspecialchars($_GET['id']) ?></h1>
    <div class="card">
        <div class="card-header">
            Scooter Details
        </div>
        <div class="card-body">
            <h5 class="card-title"><strong>Naam: </strong><?= ($scooter['naam']) ?></h5>
            <p class="card-text"><strong>Model:</strong> <?= ($scooter['model']) ?></p>
            <p class="card-text"><strong>Kleur:</strong> <?= ($scooter['kleur']) ?></p>
            <p class="card-text"><strong>Prijs:</strong> €<?= ($scooter['prijs']) ?></p>
            <p class="card-text"><strong>Merk ID:</strong> <?= ($scooter['merk_id']) ?></p>
            <?php if ($scooter['afbeelding']): ?>
                <img src="img/<?= ($scooter['afbeelding']) ?>" alt="<?= ($scooter['naam']) ?>" style="width: 200px; height: auto;">
            <?php endif; ?>
            <div class="mt-3">
                <form method="post">
                    <p>Weet je zeker dat je deze scooter wilt verwijderen?</p>
