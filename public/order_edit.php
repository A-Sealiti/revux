...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Verwerk de bewerking van de bestelling
    $purchase_id = $_POST['purchase_id'];
    $address = $_POST['address'];
    $zipcode = $_POST['zipcode'];
    $city = $_POST['city'];

    // Update de bestelling in de database
    $sql = "UPDATE purchase SET address = :address, zipcode = :zipcode, city = :city WHERE id = :id";
    $sth = $pdo->prepare($sql);
    $sth->bindParam(':address', $address);
    $sth->bindParam(':zipcode', $zipcode);
    $sth->bindParam(':city', $city);
    $sth->bindParam(':id', $purchase_id);
    $sth->execute();
...
$sth->bindParam(':id', $purchase_id);
$sth->execute();
$purchase = $sth->fetch(PDO::FETCH_ASSOC);
?>
    ...
    <div class="container mt-5">
        <h1 class="mb-4">Bestelling Bewerken</h1>
        ...
        <form action="order_edit.php" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="address" name="address" value="<?php echo ($purchase['address']); ?>" required>
            </div>
            <div class="mb-3">
                <label for="zipcode" class="form-label">Postcode</label>
                <input type="text" class="form-control" id="zipcode" name="zipcode" value="<?php echo ($purchase['zipcode']); ?>" required>
            </div>
            <div class="mb-3">
                <label for="city" class="form-label">Stad</label>
                <input type="text" class="form-control" id="city" name="city" value="<?php echo ($purchase['city']); ?>" required>
            </div>
