...
//$db = dbConnect();
$minStars = isset($_GET['minStars']) ? (int)$_GET['minStars'] : null;
$maxStars = isset($_GET['maxStars']) ? (int)$_GET['maxStars'] : null;
...
// Verkrijg gefilterde reviews
$recentReviews = getFilteredReviews($minStars, $maxStars, $startDate, $endDate, $limit);


// Haal de GET-parameters op
$zoekterm = isset($_GET['zoekterm']) ? $_GET['zoekterm'] : '';
$beschrijving = isset($_GET['beschrijving']) ? $_GET['beschrijving'] : '';
$minPrijs = isset($_GET['minPrijs']) ? $_GET['minPrijs'] : null;
$maxPrijs = isset($_GET['maxPrijs']) ? $_GET['maxPrijs'] : null;
$sorterenOptie = isset($_GET['sorteren']) ? $_GET['sorteren'] : '';
$merk_id = isset($_GET['merk_id']) ? $_GET['merk_id'] : null;

// Zoek scooters met de opgegeven filters
$scooters = zoekScooters( $zoekterm, $beschrijving, $minPrijs, $maxPrijs, $sorterenOptie, $merk_id);
...
//
//if ($isLoggedIn) {
//    // Mogelijkheid om rol van de gebruiker te controleren
//    if ($_SESSION['user']->role == "admin") {
//        echo "User is admin";
//    } else if ($_SESSION['user']->role == "member") {
//        echo "User is member";
//    }
//} else {
...
// SQL-query om het IP-adres op te slaan zonder de user_id als de gebruiker niet is ingelogd
if ($user_id !== null) {
    // Als de gebruiker is ingelogd, sla dan user_id en ip_address op
    $sql = "INSERT INTO visitor_log (user_id, ip_address) VALUES (:user_id, :ip_address)";
} else {
    // Als de gebruiker niet is ingelogd, sla dan alleen het ip_address op
    $sql = "INSERT INTO visitor_log (ip_address) VALUES (:ip_address)";
}
...
<div class="container">
    <form class="row g-2 d-flex align-items-center" method="GET">
        <div class="col-auto">
            <input class="form-control" name="beschrijving" type="text" placeholder="Zoeken op Beschrijving..."
                   value="<?= ($beschrijving) ?>">
        </div>
        <div class="col-auto">
            <input class="form-control" name="minPrijs" type="number" placeholder="Vanaf"
                   value="<?= ($minPrijs) ?>">
        </div>
        <div class="col-auto">
            <input class="form-control" name="maxPrijs" type="number" placeholder="Tot"
                   value="<?= ($maxPrijs) ?>">
        </div>
        <div class="col-auto">
            <select class="form-select" name="sorteren">
                <option value="">Sorteer op</option>
                <option value="naam_asc" <?= ($sorterenOptie === 'naam_asc') ? 'selected' : ''; ?>>Naam A-Z</option>
                <option value="naam_desc" <?= ($sorterenOptie === 'naam_desc') ? 'selected' : ''; ?>>Naam Z-A</option>
                <option value="prijs_asc" <?= ($sorterenOptie === 'prijs_asc') ? 'selected' : ''; ?>>Prijs Laag naar
                    Hoog
                </option>
                <option value="prijs_desc" <?= ($sorterenOptie === 'prijs_desc') ? 'selected' : ''; ?>>Prijs Hoog naar
                    Laag
                </option>
            </select>
        </div>
...
<div class="container mt-5">
    ...
    <div class="row">
        <?php if (empty($scooters)): ?>
            <p class="text-center">Geen scooters beschikbaar om weer te geven.</p>
        <?php else: ?>
            <?php foreach ($scooters as $data): ?>
                <div class="col-md-4 mb-4">
                    <div class="card h-100 d-flex flex-column position-relative">
                        <img src="img/<?php echo($data['afbeelding'] ?: 'placeholder.png'); ?>"
                             class="card-img-top"
                             alt="<?php echo($data['naam']); ?>">
                        <form action="like_product.php" method="POST">
                            <input type="hidden" name="scooter_id" value="<?php echo $data['id']; ?>">
                            <span class="like-button position-absolute top-0 end-0 m-2" style="cursor: pointer; z-index: 1;">
        ...
        <span class="like-count" style="display: <?php echo (isset($data['likes']) && $data['likes'] > 0) ? 'inline' : 'none'; ?>;">
            (<?php echo isset($data['likes']) ? $data['likes'] : 0; ?>)
                        ...
                        <div class="card-body d-flex flex-column">
                            <h5 class="card-title mb-3"><?php echo($data['naam'] . " " . $data['model']); ?></h5>
                        </div>
    ...
    ?>
        px-3 py-2">
        <i class="bi bi-check-circle-fill" style="color: lightgreen;"></i>
        <?php echo ($data['stock'] > 0 ? "Nog {$data['stock']} op voorraad" : "Uitverkocht"); ?>
                        ...
                        <div class="actions-wrap">
                            <div class="actions d-flex align-items-center justify-content-between">
                                <div class="price d-flex me-6">
                                    <div class="current me-6 p-3">
                                        <?php
                                        echo $data['prijs'] > 0 ? '€ ' . number_format($data['prijs'], 0, ',', '.') . ' ,- ' : '-';
                                        ?>
                                    </div>
                                </div>
                                <div>
                                    <a href="detail.php?id=<?php echo($data['id']); ?>"
                                       class="btn btn-success mb-3 m-2">
                                        Bekijk details
                                    </a>
                                </div>
                            </div>
...
<div class="container mt-5">
    <form method="GET" action=""validate>
        <div class="mb-3">
            <label for="minStars">Min. sterren:</label>
            <input type="number" name="minStars" id="minStars" min="1" max="5" aria-required="1" value="<?= ($minStars); ?>"
                   class="form-control" required>
        </div>
        <div class="mb-3">
            <label for="maxStars">Max. sterren:</label>
            <input type="number" name="maxStars" id="maxStars" min="1" max="5" value="<?= ($maxStars); ?>"
                   class="form-control" required>
            <div class="invalid-feedback">
                Voer een maximale waarde in van 5 sterren.
            </div>
        </div>
        <div class="mb-3">
...
<div class="container mt-5">
    <h2 class="text-center">Recentste Reviews</h2><br>
    <div class="row">
...
<svg width="90px" height="90px" viewBox="0 0 256 256" data-name="Layer 1" id="Layer_1"
