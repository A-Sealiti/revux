...
// Haal de merken op
//$merkenStmt = $pdo->query("SELECT id, naam FROM merk");
//$merken = $merkenStmt->fetchAll(PDO::FETCH_ASSOC);
//
//// Categorieën toewijzen
//$categorieën = [
//    1 => 1, // Vespa => Categorie 1
//    2 => 2, // Piaggio => Categorie 2
//    3 => 3  // SYM => Categorie 3
//];
//
//// Verwerk het formulier als deze is ingediend
...
//
//    // Bepaal de category_id op basis van merk_id
//    $category_id = $categorieën[$merk_id] ?? null;
//
//    // Controleer of category_id geldig is
//    if ($category_id === null) {
//        echo "Ongeldige merk_id opgegeven.";
//        exit();
//    }
//
//
...
//    if ($afbeelding['error'] === UPLOAD_ERR_OK) {
//        $validImageTypes = ['jpg', 'jpeg', 'png', 'gif'];
//        $extensie = pathinfo($afbeelding['name'], PATHINFO_EXTENSION);
//
//        if (!in_array($extensie, $validImageTypes)) {
...
//        $doelmap = __DIR__ . '/img/';
//        $bestandsnaam = pathinfo($afbeelding['name'], PATHINFO_FILENAME);
//        $uniekeBestandsnaam = $bestandsnaam . '_' . time() . '.' . $extensie;
//        $doelbestand = $doelmap . $uniekeBestandsnaam;
//
//        //  het bestand te verplaatsen
//        if (move_uploaded_file($afbeelding['tmp_name'], $doelbestand)) {
//            $stmt = $pdo->prepare("INSERT INTO scooter_brands (naam, model, jaar, kleur, afbeelding, beschrijving, prijs, merk_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
//            $stmt->execute([$naam, $model, $jaar, $kleur, $uniekeBestandsnaam, $beschrijving, $prijs, $merk_id]);
//
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $query = $pdo->prepare("SELECT * FROM scooter_brands WHERE id = :id");
    $query->bindParam(":id", $id);
    $query->execute();
    $scooter = $query->fetch(PDO::FETCH_ASSOC);

    if (!$scooter) {
        $_SESSION['message'] = "Scooter niet gevonden.";
    ...
    if (isset($_POST["update"])) {
        ...
        if (isset($_FILES['afbeelding']) && $_FILES['afbeelding']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($fileExtension, $allowedfileExtensions)) {
                // Verplaats het geüploade bestand naar de img/scooters map
                $uploadFileDir = './img/';
                ...
                if (move_uploaded_file($fileTmpPath, $dest_path)) {
                    ...
                    $_SESSION['message'] = "Er was een fout bij het uploaden van de afbeelding.";
                    header("location:admin.php");
                    exit;
                }
            } else {
                $_SESSION['message'] = "Ongeldige bestandsindeling. Alleen .jpg, .gif, .png en .jpeg zijn toegestaan.";
                header("location:admin.php");
                exit;
            }
        }

        $query = $pdo->prepare("UPDATE scooter_brands SET naam = :naam, model = :model, jaar = :jaar, kleur = :kleur, beschrijving = :beschrijving, prijs = :prijs, merk_id = :merk_id, afbeelding = :afbeelding WHERE id = :id");
        ...
        $_SESSION['message'] = "De scooter is succesvol bijgewerkt.";
        header("location:admin.php");
        exit;
    }
} else {
...
<div class="container mt-5">
    ...
    <form method="post" enctype="multipart/form-data">
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="naam" name="naam" value="<?= ($scooter['naam']) ?>" required>
        </div>
        <div class="mb-3">
            <label for="model" class="form-label">Model</label>
            <input type="text" class="form-control" id="model" name="model" value="<?= ($scooter['model']) ?>" required>
        </div>
        <div class="mb-3">
            <label for="jaar" class="form-label">Jaar</label>
            <input type="number" class="form-control" id="jaar" name="jaar" value="<?= ($scooter['jaar']) ?>" required>
        </div>
        <div class="mb-3">
            <label for="kleur" class="form-label">Kleur</label>
            <input type="text" class="form-control" id="kleur" name="kleur" value="<?= ($scooter['kleur']) ?>" required>
        </div>
        <div class="mb-3">
            <label for="beschrijving" class="form-label">Beschrijving</label>
            <textarea class="form-control" id="beschrijving" name="beschrijving" rows="3"
                      required><?= ($scooter['beschrijving']) ?></textarea>
        </div>
        <div class="mb-3">
            <label for="prijs" class="form-label">Prijs</label>
            <input type="number" class="form-control" id="prijs" name="prijs" value="<?= ($scooter['prijs']) ?>"
                   required>
        </div>
        <select class="form-control" id="merk_id" name="merk_id" required>
            <option value="1" <?= $scooter['merk_id'] == 1 ? 'selected' : '' ?>>Vespa Catagorie</option>
            <option value="2" <?= $scooter['merk_id'] == 2 ? 'selected' : '' ?>>Piaggio Catagorie </option>
            <option value="3" <?= $scooter['merk_id'] == 3 ? 'selected' : '' ?>>SYM <Catagorie></Catagorie></option>
        </select>
        <div class="mb-3">
            <label for="afbeelding" class="form-label">Afbeelding</label>
            <input type="file" class="form-control" id="afbeelding" name="afbeelding">
            <?php if ($scooter['afbeelding']): ?>
                <img src="img/<?= ($scooter['afbeelding']) ?>" alt="Current Picture"
