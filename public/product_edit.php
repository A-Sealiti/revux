...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Verwerk de bewerking van het product
    ...
    $sql = "UPDATE scooter_brands SET naam = :naam, model = :model, jaar = :jaar, kleur = :kleur, afbeelding = :afbeelding, beschrijving = :beschrijving, prijs = :prijs, voorraad = :voorraad WHERE id = :id";
    $sth = $pdo->prepare($sql);
    $sth->bindParam(':naam', $naam);
    $sth->bindParam(':model', $model);
    $sth->bindParam(':jaar', $jaar);
    $sth->bindParam(':kleur', $kleur);
    $sth->bindParam(':afbeelding', $afbeelding);
    $sth->bindParam(':beschrijving', $beschrijving);
    $sth->bindParam(':prijs', $prijs);
    $sth->bindParam(':voorraad', $voorraad);
    $sth->bindParam(':id', $product_id);
    $sth->execute();

    $bericht = "Product is succesvol bijgewerkt.";
}

// Haal de details van het product op
$product_id = $_GET['product_id'] ?? 0;
$sql = "SELECT * FROM scooter_brands WHERE id = :id";
$sth = $pdo->prepare($sql);
$sth->bindParam(':id', $product_id);
$sth->execute();
...
<div class="container mt-5">
    ...
    <form action="product_edit.php" method="post">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="jaar" name="jaar" value="<?php echo ($product['jaar']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="kleur" class="form-label">Kleur</label>
            <input type="text" class="form-control" id="kleur" name="kleur" value="<?php echo ($product['kleur']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="afbeelding" class="form-label">Afbeelding</label>
            <input type="text" class="form-control" id="afbeelding" name="afbeelding" value="<?php echo ($product['afbeelding']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="beschrijving" class="form-label">Beschrijving</label>
            <textarea class="form-control" id="beschrijving" name="beschrijving" required><?php echo ($product['beschrijving']); ?></textarea>
        </div>
        <div class="mb-3">
            <label for="prijs" class="form-label">Prijs</label>
            <input type="number" step="0.01" class="form-control" id="prijs" name="prijs" value="<?php echo ($product['prijs']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="voorraad" class="form-label">Voorraad</label>
            <input type="number" class="form-control" id="voorraad" name="voorraad" value="<?php echo ($product['stock']); ?>" required>
        </div>
