...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $query = $pdo->prepare("SELECT * FROM merk WHERE id = :id");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = $pdo->prepare("UPDATE merk SET naam = :name WHERE id = :id");
    $query->bindParam(':id', $id, PDO::PARAM_INT);
...
<div class="container mt-5">
    <h1 class="mb-4">Categorie Bijwerken</h1>
    <form action="update_category.php" method="post">
        <input type="hidden" name="id" value="<?= ($category['id']) ?>">
        <div class="mb-3">
            <label for="name" class="form-label">Naam:</label>
            <input type="text" class="form-control" id="name" name="name" value="<?= ($category['naam']) ?>" required>
        </div>
