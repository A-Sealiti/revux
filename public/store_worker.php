...
// Haal de bestellingen op voor alle gebruikers
$purchases = [];
foreach ($users as $user) {
    ...
    $sth->execute();
    $user_purchases = $sth->fetchAll(PDO::FETCH_ASSOC);
    if (!empty($user_purchases)) {
        $purchases[$user['id']] = $user_purchases;
    }
}
?>
<!doctype html>
...
<div class="container mt-5">
    <div class="row justify-content-center">
        <div class="col-12 col-md-8 col-lg-6">
            <div class="bg-light p-5 rounded shadow-sm text-center"><h1 class="mb-4">Welkom bij de Winkelmedewerker omgeving</h1>
            </div>
        </div>
    </div>
    <div class="mb-4">
        <a href="product_overzicht.php" class="btn btn-custom">Producten aanpassen</a>
    </div>
    <h2>Gebruikers en Bestellingen</h2>
    <table class="table table-bordered">
        ...
        <tr>
            ...
            <th>Achternaam</th>
            <th>Email</th>
            <th>Bestellingsstatus</th>
            <th>Inhoud van Bestellingen</th>
            <th>Bestellingen Bewerken</th>
        </tr>
        </thead>
        <tbody>
        <?php foreach ($users as $user): ?>
            <tr>
                ...
                <td>
                    <?php if (isset($purchases[$user['id']])): ?>
                        <div class="dropdown">
                            <button class="btn btn-primary btn-sm dropdown-toggle" type="button" id="dropdownStatus<?php echo $user['id']; ?>" data-bs-toggle="dropdown" aria-expanded="false">
                                Status
                            </button>
                            <ul class="dropdown-menu" aria-labelledby="dropdownStatus<?php echo $user['id']; ?>">
                                <?php foreach ($purchases[$user['id']] as $purchase): ?>
                                    <li><a class="dropdown-item" href="order_status.php?purchase_id=<?php echo ($purchase['id']); ?>">Bestelling ID: <?php echo ($purchase['id']); ?></a></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    <?php else: ?>
                ...
                <td>
                    <?php if (isset($purchases[$user['id']])): ?>
                        <div class="dropdown">
                            <button class="btn btn-primary btn-sm dropdown-toggle" type="button" id="dropdownContent<?php echo $user['id']; ?>" data-bs-toggle="dropdown" aria-expanded="false">
                                Inhoud
                            </button>
                            <ul class="dropdown-menu" aria-labelledby="dropdownContent<?php echo $user['id']; ?>">
                                <?php foreach ($purchases[$user['id']] as $purchase): ?>
                                    <li><a class="dropdown-item" href="order_content_edit.php?purchase_id=<?php echo ($purchase['id']); ?>">Bestelling ID: <?php echo ($purchase['id']); ?></a></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    <?php else: ?>
                ...
                <td>
                    <?php if (isset($purchases[$user['id']])): ?>
                        <div class="dropdown">
                            <button class="btn btn-primary btn-sm dropdown-toggle" type="button" id="dropdownEdit<?php echo $user['id']; ?>" data-bs-toggle="dropdown" aria-expanded="false">
                                Bewerken
                            </button>
                            <ul class="dropdown-menu" aria-labelledby="dropdownEdit<?php echo $user['id']; ?>">
                                <?php foreach ($purchases[$user['id']] as $purchase): ?>
                                    <li><a class="dropdown-item" href="order_edit.php?purchase_id=<?php echo ($purchase['id']); ?>">Bestelling ID: <?php echo ($purchase['id']); ?></a></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    <?php else: ?>
